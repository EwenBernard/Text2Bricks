...
class IoUValidityRewardFunc(AbstractRewardFunc): 
    ...
    def __call__(
        ...
        curr_IoU = IoU(target_img, world_img)
        iou_diff = curr_IoU - self.last_iou
        self.last_iou = curr_IoU
...
class IoUReversedValidityRewardFunc(AbstractRewardFunc): 
    def __init__(self, IoU_weight: float = 0.5 , validity_weight: float = 1.0) -> None:
        self.IoU_weight = IoU_weight
        self.validity_weight = validity_weight
        self.last_iou = 1
    ...
    def __call__(
        ...
        curr_IoU = IoU(target_img, world_img)
        if reversed:
            iou_diff = self.last_iou - curr_IoU
        else:
            iou_diff = curr_IoU - self.last_iou
        self.last_iou = curr_IoU

        iou_diff = np.sign(iou_diff) * np.exp(abs(iou_diff)) # amplify iou_diff
