...
class MLP(nn.Module):
    ...
    def __init__(self, f_fused_size, h_graph_size, hidden_dims=[128, 64, 32, 16]):
        ...
        self.fc1 = nn.Linear(f_fused_size + h_graph_size, hidden_dims[0])  # Input size is f_fused_size + h_graph_size
    ...
    def forward(self, f_fused, h_graph):
        ...
        flatten_gnn_output = h_graph.flatten(start_dim=1) # Flatten to size [batch_size, gnn_hidden_dim]

        # Concatenate the flattened features with the h_graph tensor
        combined_tensor = torch.cat((flatten_tensor_features, flatten_gnn_output), dim=1)
